...
import { Map, List, LogOut, PlusCircle, Bell, Clock, Briefcase, Star, MessageSquare } from 'lucide-react';
...
const BuyerHome = ({ 
  ...
  const categories = ['All', 'Wood', 'Metal', 'Ceramic', 'Concrete', 'Glass'];
  
  const filteredMaterials = materials.filter(m => filterCategory === 'All' || m.category === filterCategory);
  ...
  const handleFeedbackSubmit = (data) => {
      ...
      toast({ title: "Feedback Submitted", description: `Thank you for your ${feedbackStage} feedback!` });
  };
        ...
        <div className="flex gap-6 mb-8 border-b border-gray-800 overflow-x-auto">
             <button 
                ...
                className={`pb-3 px-2 text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${
                    activeTab === 'browse' ? 'border-[#3FA37C] text-[#3FA37C]' : 'border-transparent text-gray-400 hover:text-white'
                }`}
             >
                 Browse Market
             </button>
             <button 
                ...
                className={`pb-3 px-2 text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${
                    activeTab === 'deals' ? 'border-yellow-500 text-yellow-500' : 'border-transparent text-gray-400 hover:text-white'
                }`}
             >
             ...
             <button 
                ...
                className={`pb-3 px-2 text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${
                    activeTab === 'requests' ? 'border-[#60A5FA] text-[#60A5FA]' : 'border-transparent text-gray-400 hover:text-white'
                }`}
             >
                ...
                <div className="flex-1 relative">
                    {viewMode === 'list' ? (
                        ...
                        <div className="h-full min-h-[500px] rounded-xl overflow-hidden border border-gray-800 relative">
                            <MapView materials={filteredMaterials} onMarkerClick={onViewMaterial} />
                        </div>
                    )}
                </div>
            </>
                            ...
                            <div key={req.id} className="bg-[#1E2A26] rounded-xl p-6 border border-gray-800 flex flex-col md:flex-row justify-between gap-6">
                                <div>
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span className={`px-2 py-0.5 text-xs font-bold rounded uppercase ${
                                            req.status === 'Pending' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-green-500/20 text-green-500'
                                        }`}>
                                            {req.status}
                                        </span>
                                    </div>
                                ...
                                <div className="flex flex-col justify-between items-end min-w-[150px]">
                                     <div className={`px-3 py-1 rounded text-xs font-medium border ${
                                         req.urgency === 'Critical' ? 'border-red-500 text-red-500 bg-red-500/10' : 
